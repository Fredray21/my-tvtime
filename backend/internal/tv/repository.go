...
func (r *Repository) GetSeriesStatus(userID string, tmdbSeriesID int) (*SeriesRecord, error) {
	query := `
		SELECT id, user_id, tmdb_series_id, status, is_favorite 
		FROM user_series 
	...
	err := r.db.QueryRow(query, userID, tmdbSeriesID).Scan(
		...
		&series.Status, &series.IsFavorite,
	)
...
func (r *Repository) GetSeriesOrderByStatusRecords(userID string, page int, limit int) ([]SeriesRecord, error) {
	...
	for rows.Next() {
		...
		err := rows.Scan(&rec.TMDBSeriesID, &rec.Status, &rec.IsFavorite)
		if err != nil {
			return nil, err
		}
...
func (r *Repository) GetFavoriteSeriesRecords(userID string) ([]SeriesRecord, error) {
	...
	for rows.Next() {
		var s SeriesRecord
		if err := rows.Scan(&s.ID, &s.UserID, &s.TMDBSeriesID, &s.Status, &s.IsFavorite); err != nil {
			return nil, err
		}
...
func (r *Repository) SaveEpisode(userID string, tmdbSeriesID int, seasonNumber int, episodeNumber int) error {
	query := `
	...
	`
	_, err := r.db.Exec(query, userID, tmdbSeriesID, seasonNumber, episodeNumber)
	return err
}
...
func (r *Repository) GetAllWatchedEpisodes(userID string) ([]EpisodeRecord, error) {
	query := `
